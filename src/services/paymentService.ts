...
export async function getDonorsForCampaign(campaignId: string): Promise<DonorInfo[]> {
  ...
  try {
    ...
    const donorMap = new Map<string, { name: string, avatarUrl?: string | null }>();

    for (const docSnap of querySnapshot.docs) {
      const data = docSnap.data();
      const userId = data.userId;
      if (userId && !donorMap.has(userId)) {
        const userProfile = await getUserProfile(userId);
        donorMap.set(userId, {
          name: userProfile?.displayName || userProfile?.email || 'Anonymous',
          avatarUrl: userProfile?.photoURL || null,
        });
      }
    }

    const donors: DonorInfo[] = Array.from(donorMap.entries()).map(([userId, profile]) => ({
      userId,
      name: profile.name,
    ...
    if (error instanceof Error) {
      if (error.message.toLowerCase().includes("query requires an index")) {
        throw new Error(`Failed to fetch donors: The query requires a composite index for 'campaignId' and 'status'. Please create it in your Firebase console.`);
      }
      throw new Error(`Failed to fetch donors: ${error.message}`);
    }
    throw new Error('An unknown error occurred while fetching donor information.');
  }
}
